...
<head>
  ...
  <title>Foodies - Seafood & Fish Recipes</title>
  <!-- Add Poppins Font (matches first file) -->
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap">
  <style>
    * { box-sizing: border-box; }
    body {
      margin: 0;
      font-family: "Poppins", Arial, sans-serif; /* Match first file font */
      background-color: #fef6f0;
      display: flex;
      flex-direction: column;
      min-height: 100vh;
      color: #3b2418; /* Match text color */
    }
    .land {
      width: 100%;
      height: 100px;
    ...
    .nav-links a:hover { color: #ffd27f; border-bottom: 2px solid #ffd27f; } /* Match hover style */

    /* Search container (from first file) */
    .search-wrap {
      position: relative;
      display: flex;
      align-items: center;
    ...
    /* Suggestions dropdown (from first file) */
    #suggestions {
      position: absolute;
      left: 0;
      top: 56px;
    ...
    /* No results message (from first file) */
    .no-results {
      padding: 20px 14px;
      text-align: center;
      color: #c0c0c0;
    ...
    .food-loader-container { position: relative; width: 120px; height: 120px; display: flex; justify-content: center; align-items: center; }
    .pot { width: 80px; height: 80px; border-radius: 50%; border: 8px solid #FFC107; border-top-color: transparent; border-bottom-color: transparent; animation: potSpin 1.5s cubic-bezier(0.68, -0.55, 0.27, 1.55) infinite; position: absolute; }
    .steam { position: absolute; width: 30px; height: 30px; background: rgba(255, 255, 255, 0.7); border-radius: 50%; opacity: 0; animation: riseAndFade 2.5s ease-out infinite; box-shadow: 0 0 15px rgba(255, 255, 255, 0.5); }
    .steam:nth-child(2) { left: 20px; animation-delay: 0.5s; }
    .steam:nth-child(3) { left: -20px; animation-delay: 1s; }
    .cooking-icon { font-size: 50px; color: #E67E22; animation: pulse 1.5s ease-in-out infinite alternate; z-index: 10; }
    @keyframes potSpin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
    @keyframes riseAndFade { 0% { transform: translateY(0) scale(0.5); opacity: 0.8; } 50% { opacity: 0.5; } 100% { transform: translateY(-80px) scale(1.2); opacity: 0; } }
    @keyframes pulse { 0% { transform: scale(1); } 100% { transform: scale(1.1); } }
    .loader-text { color: #8B4513; margin-top: 30px; font-size: 24px; font-weight: 600; letter-spacing: 1.5px; text-shadow: 1px 1px 2px rgba(0,0,0,0.1); }
    .dots { display: inline-block; margin-left: 8px; }
    .dot { display: inline-block; width: 12px; height: 12px; background: #E67E22; border-radius: 50%; margin: 0 4px; animation: dotBounce 1.4s infinite ease-in-out both; }
    .dot:nth-child(1) { animation-delay: -0.32s; }
    .dot:nth-child(2) { animation-delay: -0.16s; }
    .dot:nth-child(3) { animation-delay: 0s; }
    @keyframes dotBounce { 0%, 80%, 100% { transform: scale(0); opacity: 0.5; } 40% { transform: scale(1); opacity: 1; } }

    /* Modal for Recipe Not Found (from first file) */
    .modal-overlay {
      position: fixed;
      top: 0;
      left: 0;
    ...
    /* Grid & Card Styles (kept from original seafood.php) */
    .grid-container {
      display: grid;
    ...
    .card-text { font-size: 14px; color: #555; margin-bottom: 15px; }
     .view-btn {
...
.view-btn:hover {
  background-color: #4d260f;
}

    /* Footer (updated to match first file) */
    .footer {
      width: 100%; 
      background-color: saddlebrown; 
      color: white;
    ...
    /* Media Queries (updated for new search layout) */
    @media (max-width: 992px) { .grid-container { grid-template-columns: repeat(2, 1fr); } }
    @media (max-width: 600px) {
      .land { flex-direction: column; align-items: flex-start; padding: 15px; height: auto; }
      .nav-section { flex-direction: column; align-items: flex-start; width: 100%; gap: 15px; }
      .search-wrap { width: 100%; min-width: auto !important; }
...
<body>
  ...
  <!-- NOT FOUND MODAL (from first file) -->
 ...
 <div class="land">
  <div class="brand">
    ...
    <div class="nav-section">
      ...
      <!-- Search Wrap from first file -->
      <div class="search-wrap" style="min-width:320px;">
        <div class="search-box" style="width:100%;max-width:420px;">
          <input id="searchInput" type="text" autocomplete="off" placeholder="Search recipes (try: seafood)">
  ...
  <!-- Seafood Recipe Cards (unchanged) -->
 <div class="grid-container">

 <div class="card">
  <img src="food_image/para/pa1.jpg" alt="Grilled Salmon with Lemon" />
  <div class="card-body">
    <h5 class="card-title">Grilled Salmon with Lemon</h5>
    <p class="card-text">Perfectly grilled salmon with a zesty lemon and herb marinade.</p>
    <a href="patra/p1.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa2.jpg" alt="Shrimp Scampi" />
  <div class="card-body">
    <h5 class="card-title">Shrimp Scampi</h5>
    <p class="card-text">Garlic butter shrimp served over pasta or rice for a delightful meal.</p>
    <a href="patra/p2.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa3.jpg" alt="Tuna Salad" />
  <div class="card-body">
    <h5 class="card-title">Tuna Salad</h5>
    <p class="card-text">Healthy salad made with fresh tuna, crisp veggies, and a light dressing.</p>
    <a href="patra/p3.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa4.webp" alt="Fish Tacos" />
  <div class="card-body">
    <h5 class="card-title">Fish Tacos</h5>
    <p class="card-text">Soft tortillas filled with seasoned fish and topped with fresh ingredients.</p>
    <a href="patra/p4.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa5.jpg" alt="Baked Cod with Herbs" />
  <div class="card-body">
    <h5 class="card-title">Baked Cod with Herbs</h5>
    <p class="card-text">Lightly seasoned cod fillets baked to perfection with aromatic herbs.</p>
    <a href="patra/p5.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa6.jpg" alt="Sushi Rolls" />
  <div class="card-body">
    <h5 class="card-title">Sushi Rolls</h5>
    <p class="card-text">Fresh sushi rolls with fish or avocado fillings for a delicious bite.</p>
    <a href="patra/p6.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa7.jpg" alt="Prawn Curry" />
  <div class="card-body">
    <h5 class="card-title">Prawn Curry</h5>
    <p class="card-text">A spicy and flavorful prawn curry made with coconut milk and aromatic spices.</p>
    <a href="patra/p7.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal8.jpg" alt="Fish Molee" />
  <div class="card-body">
    <h5 class="card-title">Fish Molee</h5>
    <p class="card-text">A mild and creamy fish curry made with coconut milk and aromatic spices.</p>
    <a href="patra/p8.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa9.jpg" alt="Fish Curry" />
  <div class="card-body">
    <h5 class="card-title">Fish Curry</h5>
    <p class="card-text">An Indian or Thai-style fish curry, rich with spices and full of flavor.</p>
    <a href="patra/p9.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal10.jpg" alt="Rava Fish Fry" />
  <div class="card-body">
    <h5 class="card-title">Rava Fish Fry</h5>
    <p class="card-text">Crispy and golden fish fillets coated with semolina and fried to perfection.</p>
    <a href="patra/p10.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal11.webp" alt="Kashmiri Fish Yakhni" />
  <div class="card-body">
    <h5 class="card-title">Kashmiri Fish Yakhni</h5>
    <p class="card-text">A traditional Kashmiri-style fish curry with a rich yogurt and aromatic spice base.</p>
    <a href="patra/p11.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal2.jpg" alt="Fish Chilli Fry (Desi Style)" />
  <div class="card-body">
    <h5 class="card-title">Fish Chilli Fry (Desi Style)</h5>
    <p class="card-text">Spicy and tangy fish fried with green chilies, ginger, and garlic.</p>
    <a href="patra/p12.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal13.jpg" alt="Goan Crab Xacuti" />
  <div class="card-body">
    <h5 class="card-title">Goan Crab Xacuti</h5>
    <p class="card-text">A flavorful Goan curry made with crab and a blend of rich spices.</p>
    <a href="patra/p13.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pa14.jpg" alt="Prawn 65" />
  <div class="card-body">
    <h5 class="card-title">Prawn 65</h5>
    <p class="card-text">A crispy and spicy fried prawn dish with a touch of Indian spices.</p>
    <a href="patra/p14.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal15.jpg" alt="Karuvadu Fry (Dry Fish Fry)" />
  <div class="card-body">
    <h5 class="card-title">Karuvadu Fry (Dry Fish Fry)</h5>
    <p class="card-text">A flavorful and crispy dry fish fry, typically enjoyed with rice or chapati.</p>
    <a href="patra/p15.php" class="view-btn">View Recipe</a>
  </div>
</div>

<div class="card">
  <img src="food_image/para/pal16.jpg" alt="Koonthal Roast" />
  <div class="card-body">
    <h5 class="card-title">Koonthal Roast</h5>
    <p class="card-text">A spicy, roasted squid dish with a blend of traditional Kerala spices.</p>
    <a href="patra/p16.php" class="view-btn">View Recipe</a>
  </div>
</div>
  ...
  <!-- Footer (unchanged except style updates) -->
  <div class="footer">
    © 2025 Foodies. All rights reserved.
  </div>

  <!-- Search & Interactive Scripts (from first file) -->
  <script>
    // Loader
    ...
    // Search suggestions data (matches seafood recipes)
   const recipes = {
 ...
 "payasam": "sweet/s16.php"
    ...
    input.addEventListener('input', () => {
      ...
      if(matches.length) {
        sugg.innerphp = matches.map((m,idx)=>
          `<div class='suggest-item \({idx===0?'suggest-active':''}' data-index='\){idx}' onclick="selectRecipe('${m}')">${m}</div>`
        ).join('');
      } else {
        sugg.innerphp = `<div class='no-results no-result-animate'>❌ No recipes found for "${q}"</div>`;
      }
    ...
    document.getElementById('searchBtn').onclick = () => {
      ...
      const page = Object.keys(recipes).find(k=>k.includes(q)) || recipes[q];
      if(!page){
        showNotFoundModal(q);
        return;
      }
